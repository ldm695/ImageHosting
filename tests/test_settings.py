...
def test_staging_timeout_validation(client, value, code):
    assert (
        client.put("/api/settings/staging-timeout", json={"staging_timeout": value}).status_code
        == code
    )
...
def test_data_dir_nonexistent_parent(client):
    r = client.put("/api/settings/data-dir", json={"data_dir": "/no/such/parent/here/child"})
    assert r.status_code == 400
